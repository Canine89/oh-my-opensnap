...
1. [**Releases**](../../releases/latest) 에서 `oh-my-snap-x.x.x.dmg` 를 받습니다.
2. `.dmg` 를 열고 **`oh-my-snap` 아이콘을 `Applications` 폴더로 드래그**합니다.
3. **처음 한 번만**: 앱을 더블클릭하면 막힐 수 있습니다. 그럴 땐
   **시스템 설정 → 개인정보 보호 및 보안** 으로 가서 아래쪽의 **`그래도 열기`** 를 누르면 실행됩니다.
4. 첫 캡처 때 뜨는 **화면 녹화 권한**을 켭니다
   (시스템 설정 → 개인정보 보호 및 보안 → 화면 및 시스템 오디오 녹화 → `oh-my-snap` ON).
...
캡처본은 **바탕화면의 `oh-my-snap` 폴더**에 PNG로 보관됩니다.
