...
2. **다른 macOS 앱을 새로 만들 때 그대로 가져갈 검증된 청사진(blueprint).** 아래의 스택 선택과 릴리스 절차는 실제로 v1.0.60까지 운영하며 다듬은 것이다. 새 앱에서 흔들리지 말고 이 기본형에서 출발하라.
...
macOS 26(Tahoe)+ **메뉴 막대 상주 화면 캡처 도구**. 영역을 픽셀 단위로 캡처 → 즉시 클립보드 복사 → 라이브러리에 모아 보기/주석/크롭. 공증 없이 무료 배포 + Sparkle 자동 업데이트.
...
| 자동 업데이트 | **Sparkle** (`SPUStandardUpdaterController(startingUpdater: true)`) + **EdDSA 서명** | App Store/공증($99) 없이 클릭 한 번 업데이트. 무결성은 EdDSA로 보장. |
| 코드 서명 | **앱 전용 자체서명 인증서** (ad-hoc 아님) | 아래 §5 참고 — 업데이트해도 TCC 권한이 유지되는 핵심. |
| 배포 | **DMG(사람) + ZIP(Sparkle)** → GitHub Releases + `appcast.xml`(raw.githubusercontent) | §4 릴리스 절차 참고. |
...
```bash
./scripts/release.sh 1.0.61            # 버전 올려 DMG+ZIP+appcast 생성 (게시는 안 함)
./scripts/release.sh 1.0.61 --publish  # 위 + 커밋/푸시 + GitHub Release 업로드
```
...
3. **자체서명 재서명** — 빌드 산출물의 서명을 제거하고 자체서명 인증서 `oh-my-opensnap`으로 `codesign --force --deep`. 검증 + Designated Requirement 출력. (인증서 없으면 중단하고 안내)
4. **패키징** — `dist/`에:
   - DMG: 스테이징 + `/Applications` 심볼릭 링크 → `hdiutil ... UDZO` (사람이 드래그-투-Applications)
   - ZIP: `ditto -c -k --keepParent` (Sparkle 업데이트용)
5. **EdDSA 서명 + appcast** — DerivedData에서 Sparkle `sign_update`를 찾아 ZIP 서명 → `appcast.xml` 생성. ZIP은 `updates/`에 버전별로 복사하고 `raw.githubusercontent.com/.../updates/...zip`로 서빙.
6. **릴리스 노트** — `CHANGELOG.md`의 `## <버전>` 섹션을 읽어 Sparkle 업데이트 창(HTML)과 GitHub 릴리스 노트(markdown) 양쪽에 사용. → **릴리스 전에 `CHANGELOG.md`에 `## <새버전>` 섹션을 먼저 추가하라.**
7. **게시(`--publish`)** — `appcast.xml` + `project.yml` + `updates/*.zip`을 `release: vX.Y.Z` 커밋으로 푸시 → 공개 ZIP 다운로드 크기 일치 검증 → `gh release create/upload`로 DMG+ZIP 업로드.

### 1회 전역 설정 (이 Mac에서 한 번, 분실 시 치명적)
- **자체서명 코드서명 인증서**: `scripts/make-signing-cert.sh --backup` → `.p12` 백업.
- **Sparkle EdDSA 키쌍**: `generate_keys`로 생성, `generate_keys -x`로 백업. 공개키는 `Info.plist`의 `SUPublicEDKey`.
- 🔐 **`.p12`와 EdDSA 개인키는 절대 git에 커밋 금지** (`.gitignore`에 `*.p12`). 분실하면 기존 사용자에게 업데이트를 못 보낸다.

---

## 5. 왜 ad-hoc이 아니라 자체서명인가 (가장 중요한 교훈)

ad-hoc 서명은 **빌드마다 cdhash가 바뀌어** macOS 26이 매 업데이트를 "다른 앱"으로 본다 → **화면 녹화 권한이 업데이트마다 풀린다.**
고정된 자체서명 인증서로 서명하면 **Designated Requirement가 인증서에 고정**되어, 같은 인증서로 서명한 모든 업데이트에서 권한이 유지된다. (팀 ID 없는 인증서라 임베드 프레임워크 로딩을 막지 않도록 라이브러리 검증식 하드닝은 피한다.)
→ 새 앱에서도 동일: **TCC 권한(화면/마이크/카메라 등)이 필요한 앱은 반드시 안정적 서명 신원을 써라.**

배포 모델: 공증 없음 → 첫 실행 때만 Gatekeeper 1회 우회(`INSTALL.md` 안내), 이후 Sparkle이 업데이트 시 quarantine을 제거하므로 우회/권한이 그대로 유지된다.
