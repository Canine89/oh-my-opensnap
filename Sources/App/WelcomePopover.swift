...
final class WelcomePopover: NSObject, NSPopoverDelegate {
    ...
    private let permissionButton = NSButton(title: "권한 허용", target: nil, action: nil)
    ...
    private func makeContent() -> NSViewController {
        ...
        let step1 = stepRow(number: 1, symbol: "keyboard",
                            title: "어디서든 \(shortcut) 로 캡처",
                            detail: "드래그해 영역을 고르거나, 창을 클릭하면 창만 잡힙니다. 단축키는 설정에서 바꿀 수 있어요.")
        ...
        let step2 = stepRow(number: 2, symbol: "lock.shield",
                            title: "화면 녹화 권한 한 번 허용",
                            detail: "macOS가 묻는 표준 권한입니다. 캡처는 이 권한 없이는 동작하지 않아요.",
                            accessory: permissionRow)

        let step3 = stepRow(number: 3, symbol: "doc.on.clipboard",
                            title: "캡처하면 바로 붙여넣기",
                            detail: "결과는 클립보드에 즉시 복사되고, 라이브러리에 모여 나중에 주석·크롭할 수 있습니다.")

        let done = NSButton(title: "시작하기", target: self, action: #selector(close))
        ...
        let stack = NSStackView(views: [headerRow, step1, step2, step3, buttonRow])
        stack.orientation = .vertical
        stack.alignment = .leading
        stack.spacing = 14
        stack.setCustomSpacing(18, after: headerRow)
    ...
    private func refreshPermission() {
        if ScreenCapturePermission.isGranted {
            permissionStatus.stringValue = "✓ 허용됨"
            permissionStatus.textColor = .systemGreen
            permissionButton.isHidden = true
        } else {
            permissionStatus.stringValue = "아직 허용되지 않음"
